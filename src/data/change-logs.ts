      ...
      changes: [
        {
          ...
          changes: [
            ...
            t('修正了宏管理界面输入框在界面语言设置为日语/英语时的显示效果。'),
      ...
      changes: [
        {
          ...
          changes: [
            t('实装全新功能「生产宏管理」。')
              ...
              + '<br>※ ' + t('伴随这一改动，“{f1}”中追加了“{f2}”选项卡。', {
                f1: t('偏好设置'),
                f2: t('生产宏'),
              }) + t('相应的，原有的“{f}”选项卡将被撤除，其下的设置项将移动到其他选项卡中。', {
                f: t('导入/导出')
              })
            ,
            t('国服数据库更新至{ver}，', {
        ...
        },
        {
          name: groupName.bugfix,
          changes: [
            t('修正了克扎玛乌卡湿地的地图数据，现在会正常显示水果码头传送点。'),
            t('修复了「成本/收益分析」和「加入工作流」弹窗中物品信息图标的复制功能没有正常工作的问题。'),
            t('修复了制作报表内物品名称过长时表格各列会错位的问题。'),
            t('修复了赞助者名单有时会因缓存而没有及时更新的问题。'),
            t('修正了管理工作流弹窗中一些内容组件的渲染问题。')
              + '<br>※ ' + t('此项是对2.2.3版本中相应修正的进一步处理。'),
          ]
        },
        {
          name: groupName.feature,
          changes: [
            ...
            t('在制作报表中左键单击任意物品行后，它和它的制作素材会被高亮展示。')
              + '<br>※ ' + t('如果不需要这一功能，可以在“{f1}”的“{f2}”选项卡中打开设置项“{f3}”。', {
                f1: t('偏好设置'),
                f2: t('制作报表'),
                f3: t('禁用高亮展示关联素材')
              })
              + '<br>' + renderImg('http://lsky.nbb.fan/i/2025/05/21/682d9f225fe9d.png'),
            t('现在在将物品加入工作流时，可以自由选择加入模式。')
              + '<br>※ ' + t('模式选择为“累加”即与之前版本的逻辑一致。可以在弹窗中查看详细说明。')
      ...
      changes: [
        {
          ...
          changes: [
            ...
            t('修复食药计算与工作流的统计中，部分道具会在采集统计和点数兑换统计中重复出现的问题。'),
            t('修正了工作流报表中子标题的背景色。'),
            t('修正主界面选择部件区域操作按钮的显示逻辑。'),
          ]
        },
        {
          name: groupName.feature,
          changes: [
            ...
            t('各个功能的默认设置有所调整。') + tipper + t('仅对第一次使用HqHelper的用户生效。'),
            t('偏好设置中，部分设置项修改之后不再需要刷新页面才能生效。'),
            t('优化了主界面右键职业按钮的展开菜单的显示效果。'),
          ]
        }
      ]
    },
