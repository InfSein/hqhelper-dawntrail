...
### 重要变更
1. 实装全新功能「物品价格详表」。
   - 现可点击“成本/收益分析”和物品悬浮窗中的“详表”按钮来查看这个物品的价格详表。
2. 正式启用全新域名 <hqhelper.com> 。
   - 现可通过顶级域名访问和使用HqHelper。
   - ※ 旧域名仍将保持维护。
...
### 问题修复
1. 修复了主界面统计区域在特定场合会因为物品名称过长而发生界面元素错位的问题。
2. 修复了“成本/收益分析”弹窗在物品名称过长时表格会发生错位的问题。
3. 修复了移动端特定物品的物品按钮会过长的问题。
4. 修复了移动端专业版制作报表在物品名称过长时表格会发生错位的问题。
5. 修复了移动端专业版制作报表在界面语言设置为日文/英文时表格会发生错位的问题。
6. 修复了移动端“导入/导出Excel”弹窗在界面语言设置为日文时界面元素错位的问题。
7. 修复了移动端采集时钟页面“配置时钟”区域的部分文本在界面语言设置为日文时会发生错位的问题。
8. 修复了移动端工作流页面“挑选物品”区域的已选物品单元格在物品名称过长时表格会发生错位的问题。
9. 修正／优化了“选择版本”区域帮助按钮弹窗在移动端的显示效果。

### 功能更新
1. 宏管理中追加多选操作功能。
在“批量操作”的下拉菜单中点击“选择…”，即可进入选择模式，自由多选宏进行分享/删除等操作。
...
2. 推荐流程中追加了“路径总览”功能。
3. 对生产宏管理功能进行了多项优化。
  - 接入CAC工序码标准，现可通过CAC工序码导入和分享制作手法。
  - 追加了工序编辑器，今后可以更加便利地编辑宏内容。
4. 对物品价格及其关联的成本/收益分析功能进行了多项优化。
  - 提升了一次性请求较多物品价格时的稳定性，并且优化了各处细节。
  - 现可选择繁中服的各个服务器作为物品价格服务器。
  - 现在成本/收益分析中的表格支持按单价/总价进行即时排序。
  - 现在不可交易物品的价格不再显示为“未知”，而是“不可交易”，从而和真正价格未知的物品区分开。
  - 现在成本/收益物品中含有不可交易/未获取到价格的物品时仍会计算有价格物品的总成本/收益，这时还会显示对应的提示文本。
5. 重制了客户端下载页面。

### 功能更新
1. 现在在背包库存的搜索框选择已有物品时，会滚动到其所在的位置并高亮显示一段时间。
2. 设置项“制作物品排序规则”追加可选项“制作笔记顺序（搜索）”。
...
### 功能更新
1. 提高了采集地图的清晰度。此外，现在光标悬浮在传送点上时，会提示该传送点的名称。


## 2.4.3 (2026-01-04)

### 重要变更
1. 可采集道具的物品悬浮窗追加了采集难度系数和采集点特效的信息。

### 问题修复
1. 修复了少数情况下可能会将背包库存的部分物品数量设置为负数，并因此无法查看和修改该物品库存的问题。
2. 修复了宏管理中，宏内容在拖拽排序后可能会出现不明元素并进而导致页面崩溃的问题。
3. 修复了部分UI文本在切换界面语言后不会立即跟随设置变更显示语言的问题。

### 功能更新
1. “偏好设置”的“物品价格”选项卡中追加了设置项“成本/收益分析中物品排序规则”。
   现可按照物品价格进行排序。
2. 进行了一些细节优化。
...
### 问题修复
1. 修复了在偏好设置中将「推荐流程」的「制作物品排序规则」设置为「制作笔记顺序」后，部分物品的顺序仍会与游戏内顺序不一致的问题。
2. 修复了将宏生成模式设置为多行模式后，生成的宏内容复制到游戏内只能粘贴出第一行的问题。
3. 修复了将宏生成模式设置为多行模式后，生成宏的部分内容未能被正确附加宏前缀的问题。
4. 修复了部分道具仍旧显示可以使用亚拉戈天道神典石进行兑换的问题。

### 功能更新
1. 「背包库存」中现在可以设置碎晶／水晶／晶簇的库存数量。
